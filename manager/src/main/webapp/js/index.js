    ...
    function fetchClusterThroughputData() {
        getJsonFromServerWithPost("clusterInstantPerfValue.ajax",
            encodeFormData({
                ...
                logger.debug(returnedJson);
                while (last.length != 0) {
                    last.shift();
                }

                for (i = 0; i < returnedJson.length; i++) {
                    loadDigitalInfo("cluster_recv_", returnedJson[i]["id"], returnedJson[i]["netIn_deriv"]);
                    loadDigitalInfo("cluster_send_", returnedJson[i]["id"], returnedJson[i]["netOut_deriv"]);
                    loadDigitalInfo("total_qps_count_", returnedJson[i]["id"], returnedJson[i]["reCount_deriv"]);
                    loadDigitalInfo("total_connection_count_", returnedJson[i]["id"], returnedJson[i]["conCount"]);

                    var count = returnedJson[i]["activeCount"] + "/<font color='red'>" + returnedJson[i]["errorCount"] + "</font>/" + returnedJson[i]["totalCount"];

                    loadDigitalInfo("cobar_count_", returnedJson[i]["id"], count);
                    loadDigitalInfo("schema_count_", returnedJson[i]["id"], returnedJson[i]["schemaCount"]);
